...
export function Dashboard() {
  ...
  const onSubmit = async (data: z.infer<typeof trackingSchema>) => {
    ...
    const destination = selectedRoute?.stops.find(
      (s) => s.id === data.destination
    );
    ...
    if (origin && destination) {
      try {
        ...
        setEta(
          `Llegada a las ${arrivalTime.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          })}`
        );
        ...
        <header className="sticky top-0 z-10 flex h-16 items-center gap-4 border-b bg-background/80 backdrop-blur-sm px-4 md:px-6">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Button variant="ghost" size="icon" className="rounded-full">
              ...
              <span className="sr-only">PQR</span>
            </Button>
            <Button variant="ghost" size="icon" className="rounded-full">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Resumen generado por IA de las incidencias recientes.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {summaryLoading ? (
            ...
            <Card className="shadow-lg">
              <CardHeader>
                <CardTitle className="font-headline">
                  {isTracking ? "Viaje en Progreso" : "Iniciar Viaje"}
                </CardTitle>
                <CardDescription>
                  {isTracking
                        ...
                        <p className="text-sm text-muted-foreground flex items-center gap-2">
                          ...
                          <span>
                            {form.getValues("destination") &&
                              selectedRoute?.stops.find(
                                (s) => s.id === form.getValues("destination")
                              )?.name}
                          </span>
                        </p>
                      </div>
